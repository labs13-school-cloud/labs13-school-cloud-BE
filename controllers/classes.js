...
router 
    ...
    .post(async (req, res) => {
        ...
        const { class_name, grade_level, subject, number_of_students, teacher_name } = req.body;

        // Add the new class  to the database
        const newClass = await Classes.add({ class_name, grade_level, subject, number_of_students, teacher_name });
...
router
    ...
    .get(async (req, res) => {
        ...
        if(!classes) {
            return res.status(404).json({ message: "Sorry! The class you are looking for does not exist." })
        }
        ...
        if(!volunteers.length) {
            res.status(404).json({ message: "This class currently does not have any volunteers assigned to it." })
        }
...
router
    ...
    .post(async (req, res) => {
        ...
        if(relation) {
            return res.status(400).json({ message: "This volunteer is already assigned to this class." })
        }
...
router
    ...
    .delete( async (req, res) => {
        ...
        if(!volunteer) {
            return res.status(404).json({ message: "This volunteer does not belong to this class." })
        }
